...
class NoAdvertise:
    def __init__(self, as_patt, comm_patt, neg_patt, adv_patt, clean_patt, info_patt, c_type):
    ...
    def format_line(self, asn, line):
        try:
            ...
            new_line = self.advertise.sub(' ', new_line)
            new_line = new_line.replace('  ', ' ')
    ...
    def save_file(com_dic):
        with open(com_no_advertise, 'w') as n_adv:
            ast = 0
            comt = 0
            text = ''
            as_set = set()
            for asn in com_dic:
                for _type in com_dic[asn]:
                    if not _type.startswith('NO-ADVERTISE'): continue
            ...
            if text:
                n_adv.write(text)
        print('NO_ADV: TOTAL AS:{}, TOTAL COMMUNITIES: {}'.format(ast, comt))
