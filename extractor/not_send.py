...
class NotSend:
    def __init__(self, as_patt, comm_patt, neg_patt, snd_patt, clean_patt, info_patt, c_type):
    ...
    def format_line(self, asn, line):
        try:
            ...
            new_line = self.send.sub(' ', new_line)
            new_line = new_line.replace('  ', ' ')
    ...
    def save_file(com_dic):
        with open(com_not_send, 'w') as n_exp:
            ast = 0
            comt = 0
            text = ''
            as_set = set()
            for asn in com_dic:
                for _type in com_dic[asn]:
                    if not _type.startswith('NOT-SEND'): continue
            ...
            if text:
                n_exp.write(text)
        print('NOT-SEND: TOTAL AS:{}, TOTAL COMMUNITIES: {}'.format(ast, comt))