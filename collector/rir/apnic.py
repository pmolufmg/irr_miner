...
class Apnic:
    ...
    def collect(self, asn):
        if asn.isnumeric():
            try:
                irr_file = os.path.join(apnic_dir, asn + '.txt')
    ...
    def set_list(self):
        if not os.path.isfile(apnic_list):
            self.set_new_list()
            new = open(apnic_list, 'w')
            new.write(str(self.as_list))
            new.close()
        else:
            try:
                specific_list = ast.literal_eval(open(apnic_list).read())
    ...
    def get_last(self):
        if not os.path.isfile(apnic_log):
            open(apnic_log, 'a').close()
            return self.as_list[0]
        else:
            with open(apnic_log) as last_file:
    ...
    def save_data(self):
        ...
        file = open(apnic_log, 'a')
        file.write(text)
        file.close()

    def save_as_list(self):
        file = open(apnic_list, 'w')
