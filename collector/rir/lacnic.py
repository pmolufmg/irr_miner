...
class Lacnic:
    ...
    def denied(self, text, asn, file):
        ...
        if size < 6:
            ...
            try:
                exceeded = lines[5] + '|' + lines[15]
                return self.set_error_type(asn, file, lines[0], exceeded, lines[9])
            except IndexError:
    ...
    def collect(self, asn):
        if asn.isnumeric():
            try:
                irr_file = os.path.join(lacnic_dir, asn + '.txt')
    ...
    def set_list(self):
        if not os.path.isfile(lacnic_list):
            self.set_new_list()
            new = open(lacnic_list, 'w')
            new.write(str(self.as_list))
            new.close()
        else:
            try:
                specific_list = ast.literal_eval(open(lacnic_list).read())
    ...
    def get_last(self):
        if not os.path.isfile(lacnic_log):
            open(lacnic_log, 'a').close()
            return self.as_list[0]
        else:
            with open(lacnic_log) as last_file:
                ...
                try:
                    ...
                    queries = int(queries)
                    self.num_of_queries = queries if queries > 6000 else 20000
    ...
    def set_error_type(self, asn, as_file, case1, case2='None', case3 = 'None'):
        if re.search('error', case1, re.IGNORECASE):
        ...
        elif re.search('exceeded', case2, re.IGNORECASE):
            #print('2: {}'.format(case2))
            self.error_type = 3
            case = case3.replace(',', '')
            case = case.split('|')
            self.error_str = case[0] if 'exceeded' in case[0] else case[-1]
            return True

        elif re.search('error', case3, re.IGNORECASE):
            print('3: {}'.format(case3))
            self.error_type = 2
            ...
            #print('none of them: 1: {}\n2: {}\n3: {}'.format(case1, case2, case3))
            return False
    ...
    def save_data(self):
        ...
        file = open(lacnic_log, 'a')
        file.write(text)
        file.close()

    def save_as_list(self):
        file = open(lacnic_list, 'w')
